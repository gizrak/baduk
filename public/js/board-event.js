define([ 'jquery' ], function($) {
	...
	BoardEvent = function() {
		/**
		 * Left click will be handled to put stone. If option is 'mouse', right
		 * will be black color.
		 */
		...
		/**
		 * Right click will be ignored except when color option is 'mouse' If
		 * option is 'mouse', right will be black color.
		 */
		...
		function handleEvent(e, color) {
			...
			if (color) {
				stoneColor = color;
			} else if (typeof optionColor === 'undefined' || optionColor === ''
					|| optionColor === 'auto') {
